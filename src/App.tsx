    ...
    <div className="min-h-screen flex flex-col md:flex-row font-sans pb-16 md:pb-0"
      ...
      {showAccountModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="rounded-xl p-6 w-full max-w-sm" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
              <Wallet className="w-5 h-5 text-blue-500" /> Create Account
            </h2>
            <form onSubmit={handleCreateAccount} className="space-y-4">
              <div>
                <label className="block text-sm mb-1" style={{ color: 'var(--text-muted)' }}>Account Name</label>
                <input
                  required type="text" value={newAccountName}
                  onChange={e => setNewAccountName(e.target.value)}
                  className="input-field"
                  placeholder="e.g. My IDR Account"
                />
              </div>
              <div>
                <label className="block text-sm mb-1" style={{ color: 'var(--text-muted)' }}>Currency</label>
                <select
          ...
          {accounts.length > 0 && (
            <>
              <select 
                ...
                className="input-field py-1 text-sm w-32 truncate"
              ...
              <button onClick={() => setShowAccountModal(true)} className="p-1.5 rounded-lg border text-blue-400 transition-colors"
      ...
      >
        ...
        <div className="hidden md:flex items-center justify-between mb-10 pb-5" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          >
            {theme === 'dark'
              ...
              : <Moon className="w-4 h-4 group-hover:text-indigo-400 transition-colors" />}
          </button>
        </div>

        {/* Desktop Account Selector */}
        <div className="hidden md:block mb-6">
          <label className="text-xs uppercase font-semibold mb-2 block" style={{ color: 'var(--text-faint)' }}>Active Account</label>
          <div className="flex items-center gap-2">
            <select 
              value={selectedAccountId} 
              onChange={e => setSelectedAccountId(e.target.value)}
              className="input-field flex-1 text-sm py-2 cursor-pointer"
            >
              {accounts.map(acc => (
                <option key={acc.id} value={acc.id}>{acc.name}</option>
              ))}
            </select>
            <button onClick={() => setShowAccountModal(true)} title="Add New Account"
                    className="p-2.5 rounded-lg border text-blue-400 transition-colors"
                    style={{ backgroundColor: 'var(--bg-base)', borderColor: 'var(--border)' }}
                    onMouseEnter={e => (e.currentTarget.style.backgroundColor = 'var(--hover-bg)')}
        ...
        <div className="flex md:flex-col gap-1 md:gap-2 w-full">
            ...
            <button
              ...
              className={`flex-1 md:w-full flex flex-col md:flex-row items-center justify-center md:justify-start gap-1 md:gap-3 p-2 md:px-4 md:py-3 rounded-lg transition-all ${
                activeTab === id
                  ...
                  : 'md:hover:bg-[var(--hover-bg)]'
              }`}
              style={activeTab !== id ? { color: 'var(--text-muted)' } : undefined}
            >
          ...
          >
            <BarChart2 className="w-5 h-5" style={{ color: 'var(--text-faint)' }} />
            <span className="text-[10px] md:text-sm font-medium" style={{ color: 'var(--text-faint)' }}>Analytics</span>
          ...
          <button 
            ...
            style={activeTab !== 'import' ? { color: 'var(--text-muted)' } : undefined}
          >
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto w-full">
        <div className="p-4 md:p-8 max-w-6xl mx-auto">
          ...
          <header className="flex justify-between items-center mb-6 md:mb-8 pb-4 md:pb-5 text-left"
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-white to-gray-400 dark:from-white dark:to-gray-400 bg-clip-text text-transparent"
            ...
            {activeAccount && (
              <button 
                ...
                className="text-xs flex items-center gap-1.5 transition-colors p-1.5 md:p-2 rounded-lg border"
          ...
          <Suspense fallback={
            ...
            {!activeAccount || accounts.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-8 text-center h-[50vh] border border-dashed rounded-xl"
              ...
              <div className="max-w-3xl mx-auto">
                {/* Sub-tab bar */}
                <div className="flex gap-1 p-1 mb-6 rounded-xl border" style={{ backgroundColor: 'var(--bg-base)', borderColor: 'var(--border)' }}>
                  <button
                    ...
                    style={inputTab !== 'trade' ? { color: 'var(--text-muted)' } : undefined}
                  >
                  ...
                  <button
                    ...
                    style={inputTab !== 'balance' ? { color: 'var(--text-muted)' } : undefined}
                  >
